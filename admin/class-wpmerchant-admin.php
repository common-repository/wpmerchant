...
class Wpmerchant_Admin {
	...
	public function __construct( $plugin_name, $version ) {
		...
		/*add_action('add_meta_boxes_wpmerchant_plans', array( $this, 'setup_plan_metaboxes' ));
		add_action( 'save_post_wpmerchant_plans',  array( $this, $this->plugin_name.'_plans_save_meta_box_data') );
		
		add_action('add_meta_boxes_wpm_affiliates', array( $this, 'setup_affiliate_metaboxes' ));
		add_action( 'save_post_wpm_affiliates',  array( $this, $this->plugin_name.'_affiliates_save_meta_box_data') );*/
		...
		add_shortcode( 'wpmerchant_cost', array( $this, 'costShortcode' ));
		...
		// include the stripe functionality in this js file
		  ...
		  wp_localize_script( $this->plugin_name, 'ajax_object', 
		  	array( 
				...
				'add_plan_nonce'=> $addPlanNonce,
				'get_plan_nonce'=> $getPlanNonce,
	...
	public function setup_product_metaboxes(){
		...
		add_meta_box( $this->plugin_name.'_product_button_shortcode_meta_box', 'Buy Button Shortcode', array($this,$this->plugin_name.'_product_button_shortcode_meta_box'), 'wpmerchant_products', 'normal','core' );
	...
	*/
	...
	/**
	 * Product Button Shortcode Meta Box Callback
	 * 
	 * @since    1.0.0
	 */
	function wpmerchant_product_button_shortcode_meta_box( $post ) {
		/*
	...
	function wpmerchant_product_data_meta_box( $post ) {
		...
		unset($args);
		$args = array('dashicon'=>'dashicons-email','class'=>'email_tab', 'tab_text'=>'Email','link_href'=>'#email_product_fields','link_title'=>'Email Product Data');
		$email_tab = $this->getPostMetaTab($args);
		$email_tab_id = 'email_product_fields';
		echo '<div class="product_container">
				<ul class="product_container_tabs wpm_tabs">'.$general_tab.$inventory_tab.$email_tab.'</ul>
		...
		echo '</li></ul></div></div>';
		echo '<div id="'.$email_tab_id.'" class="wpm_hide tab_content"><div class="product_field_containers"><ul class="wpmerchant_product_data_metabox">';
		echo '<li><label for="'.$this->plugin_name.'_post_purchase_subject">';
				_e( 'Post Purchase Subject', $this->plugin_name.'_post_purchase_subject' );
				echo '</label>';
				unset($args);
			  	$args = array (
			              'type'      => 'input',
						  'subtype'	  => 'text',
						  'id'	  => $this->plugin_name.'_post_purchase_subject',
						  'name'	  => $this->plugin_name.'_post_purchase_subject',
						  'required' => '',
						  'get_options_list' => '',
						  'value_type'=>'normal',
						  'wp_data' => 'post_meta',
				...
				$this->wpmerchant_render_settings_field($args);
				echo '</li>';
				$post_purchase_body= get_post_meta( $post->ID, $this->plugin_name.'_post_purchase_body', true );
						// DON'T WANT APPLY FILTERS BECAUSE WE WANT SAVED SHORTCODES TO BE SHOWN - IT WON'T BE IF THE CONTENT IS RUN THROUGH THIS
				//$sold_out_message = apply_filters('the_content', $sold_out_message); 
				$args = array(
				'textarea_name' => $this->plugin_name.'_post_purchase_body',
				); 
				echo '<li><label for="'.$this->plugin_name.'_post_purchase_body">';
						_e( 'Post Purchase Body', $this->plugin_name.'_post_purchase_body' );
						echo '</label>';
				wp_editor( $post_purchase_body, 'wpmerchant_post_purchase_body_editor',$args); 
	...
	function wpmerchant_products_save_meta_box_data( $post_id ) {
		...
		$post_purchase_body = wp_kses_post( $_POST[$this->plugin_name.'_post_purchase_body'] );
		$post_purchase_subject = sanitize_text_field( $_POST[$this->plugin_name.'_post_purchase_subject'] );
		...
		update_post_meta( $post_id, $this->plugin_name.'_post_purchase_subject', $post_purchase_subject );
		update_post_meta( $post_id, $this->plugin_name.'_post_purchase_body', $post_purchase_body );
	}
	/**
		...
		if($a['products']) {
			...
			if(strstr($a['products'],',')){
				...
				foreach($product_ids AS $key=>$value){
					...
					$cost = get_post_meta( $p, 'wpmerchant_cost', true );
				...
				$amount = get_post_meta( $a['products'], 'wpmerchant_cost', true )*100;
	...
	/**
	 * Product/plan Cost Shortcode Functionality
	 *
	 * @since    1.0.0
	 */
	public function costShortcode( $atts, $content = "" ) {
		// [wpmerchant_form plans="" button_text="Purchase" products="1196" classes="nectar-button handdrawn-arrow large accent-color regular-button" style="visibility: visible; color: rgb(255, 255, 255) !important;" coupon="true" hidden="data-currency:usd;data-description:blah blah blah;"][/wpmerchant_form]
		$a = shortcode_atts( array(
			          'plans' => '',
					  'products'=>'',
					  'currency'=>'',
		...
		if($a['products']) {
			...
			if(strstr($a['products'],',')){
				...
				$amount = get_post_meta( $a['products'], 'wpmerchant_cost', true );
				//$description = get_post_meta( $a['product_id'], $this->plugin_name.'_description', true );
				$description = get_the_title( $a['products'] );
				$product_ids[] = array('quantity'=>1, 'id'=>$a['products']);
			}
		...
		if(!isset($a['currency'])) {
			$currency1 = get_option( 'wpmerchant_currency' );
			$currency = $this->get_currency_details($currency1);
			$currency_symbol = ($currency['symbol']) ? $currency['symbol'] : $currency['value'];
		} else {
			$currency1 = $this->get_currency_details($a['currency']);
			$currency_symbol = ($currency1['symbol']) ? $currency1['symbol'] : $currency1['value'];
		}
	...
	**/
	...
	public function wpmerchant_render_mailchimp_connected(){
		...
		echo '<a class="btn button mailchimp-connected"><span>| Connected</span></a>&nbsp;&nbsp;<a id="mailchimp-log-out" style="cursor:pointer;">Disconnect?</a>';
	}
	public function wpmerchant_render_stripe_connected(){
		...
		echo '<a class="btn button stripe-connected"><span>| Connected</span></a>&nbsp;&nbsp;<a id="stripe-log-out" style="cursor:pointer;">Disconnect?</a>';
	}
	/**
	...
	**/
	 ...
	 * @since 1.0.0
	 */
	public function wpmerchant_display_login_settings() {
	  echo '<p>Enter in the following information to customize your site\'s login page.</p>';
	} 
	/**
	 * This function provides a simple description for the Stripe Payments Options page.
	 * This function is being passed as a parameter in the add_settings_section function.
	...
	public function wpmerchant_display_post_checkout() {
	  echo '<p>After a user successfully purchases a product or plan, you can redirect them to a page that you enter in below OR you can add a short message to display to the user.</p>';
	}
	/**
	 * This function provides a simple description for the Stripe Payments Options page.
	 * This function is being passed as a parameter in the add_settings_section function.
...
add_submenu_page( 'my-top-level-slug', 'My Custom Submenu Page', 'My Custom Submenu Page', 'manage_options', 'my-secondary-slug');
	  **/
	  ...
	  add_submenu_page( $this->plugin_name, 'WPMerchant Dashboard', 'Dashboard', 'administrator', $this->plugin_name);
	...
	*/
	...
	public function wpmerchant_render_mailchimp_connect(){
  	  $saveEmailAPINonce = wp_create_nonce( "wpmerchant_save_email_api" );
  	  update_option( 'wpmerchant_save_email_api_nonce', $saveEmailAPINonce );
	  $user_ID = get_current_user_id();
	  $ajax_url = admin_url( 'admin-ajax.php' );
	  $ref_url = admin_url( 'admin.php?page=wpmerchant-settings&tab=emails' );
	  $href = 'https://www.wpmerchant.com/mailchimp-connect/auth/?ajax_url='.urlencode($ajax_url).'&ref_url='.urlencode($ref_url).'&action=wpmerchant_save_email_api&security='.urlencode($saveEmailAPINonce).'&user_id='.urlencode($user_ID);
		 echo '<a class="btn button mailchimp-login" href="'.$href.'"><span>| Connect</span></a>';
	}
	/**
	 * Render Mailchimp Connect Button
	 *
	...
	public function wpmerchant_render_stripe_connect(){
	  $savePaymentAPINonce = wp_create_nonce( "wpmerchant_save_payment_api" );
	  update_option( 'wpmerchant_save_payment_api_nonce', $savePaymentAPINonce );
	  $user_ID = get_current_user_id();
	  $ajax_url = admin_url( 'admin-ajax.php' );
	  $ref_url = admin_url( 'admin.php?page=wpmerchant-settings&tab=payment' );
	  $href = 'https://www.wpmerchant.com/stripe-connect/auth/?ajax_url='.urlencode($ajax_url).'&ref_url='.urlencode($ref_url).'&action=wpmerchant_save_payment_api&security='.urlencode($savePaymentAPINonce).'&user_id='.urlencode($user_ID);
		 echo '<a class="btn button stripe-login" href="'.$href.'"><span>| Connect</span></a>';
	}
	/**
	...
	**/
	...
	public function wpmerchant_validate_secret_api_key( $input ) {
	  ...
	  if( !empty( $input )) {
	    ...
	    add_settings_error( $this->plugin_name.'_stripe_live_secret_key', 'invalid-api-key', __( 'Make sure you add your API Keys to connect to Stripe.', $this->plugin_name ) );
	  }
	  return $output;
	} 
	/**
